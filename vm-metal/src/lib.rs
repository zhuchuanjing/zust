...
use crate::{
    context::MetalCompiler,
    externs::register_externs,
    symbols::{collect_type_defs, collect_type_names, collect_user_fns, collect_workgroup_statics},
};

pub fn compile_source(source: impl AsRef<[u8]>, module_name: &str, fn_name: &str) -> Result<Kernel> {
    compile_source_with_externs(source, module_name, fn_name, metal_builtins())
}
...
pub fn compile_source_with_workgroup_size(source: impl AsRef<[u8]>, module_name: &str, fn_name: &str, workgroup_size: [u32; 3]) -> Result<Kernel> {
    compile_source_with_externs_and_workgroup_size(source, module_name, fn_name, metal_builtins(), workgroup_size)
}
...
pub fn compile_file_with_generic_args_and_workgroup_size(path: impl AsRef<Path>, module_name: &str, fn_name: &str, generic_args: &[Type], workgroup_size: [u32; 3]) -> Result<Kernel> {
    ...
    let externs = register_externs(&mut compiler, metal_builtins())?;
...
pub fn compile_function(compiler: &mut Compiler, module_name: &str, fn_name: &str) -> Result<Kernel> {
    let externs = register_externs(compiler, metal_builtins())?;
...
pub fn compile_function_with_externs_generic_args_and_workgroup_size(
    ...
    let builder = MetalCompiler::new(externs, user_fns, type_defs, type_names, workgroup_static_tys, compiler.clone(), workgroup_size);
    let metal = builder.compile_kernel(&arg_tys, ret_ty.clone(), &body)?;
    Ok(Kernel { metal, entry: "zust_main".into(), arg_tys, ret_ty, workgroup_size })
}
