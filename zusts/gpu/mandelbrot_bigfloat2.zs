...
fn escape_value_bigfloat2(x: f32, y: f32, max_iter: u32) {
    let iter = 0u32;
    let zx = bigfloat::BigFloat<2>::from_f32(0.0f32);
    let zy = bigfloat::BigFloat<2>::from_f32(0.0f32);
    let x_bf = bigfloat::BigFloat<2>::from_f32(x);
    let y_bf = bigfloat::BigFloat<2>::from_f32(y);
    let two_bf = bigfloat::BigFloat<2>::from_f32(2.0f32);
    let escape_radius2 = bigfloat::BigFloat<2>::from_f32(4.0f32);
...
pub fn main(params: Params, buf: Vec<f32>) {
    ...
    if px < sample_width && py < sample_height {
        ...
        buf[pos] = escape_value_bigfloat2(x, y, params.max_iter);
    }
}
