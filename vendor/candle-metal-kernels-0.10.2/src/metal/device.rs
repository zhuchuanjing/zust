...
use objc2::{rc::Retained, runtime::AnyObject, runtime::ProtocolObject, Message};
...
impl Device {
    ...
    pub fn all() -> Vec<Self> {
        let mut devices = MTLCopyAllDevices()
            .iter()
            .map(|raw| Device { raw: raw.retain() })
            .collect::<Vec<_>>();

        if devices.is_empty() {
            devices.extend(MTLCreateSystemDefaultDevice().map(|raw| Device { raw }));
        }
