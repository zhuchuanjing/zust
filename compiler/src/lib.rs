        ...
        match pat.kind {
            ...
            PatternKind::Tuple(pats) => {
                if let Type::Tuple(tys) = &expr_ty {
                    ...
                    Err(Self::semantic_error(pat.span, format!("元组模式 {:?} 与类型 {:?} 不匹配", pats, expr_ty)))
                }
            }
    ...
    fn compile_stmt(&mut self, stmt: Stmt, compiled: &mut Vec<Stmt>, cap: &mut Capture) -> Result<()> {
        ...
        match stmt.kind {
            StmtKind::Let { pat, value } => {
                ...
                let pattern_expr_ty = if matches!(pat.kind, PatternKind::List { .. } | PatternKind::Tuple(_)) {
                    if let StmtKind::Expr(expr, _) = &value.kind { Some(if matches!(expr.kind, ExprKind::List(_) | ExprKind::Tuple(_)) { expr.get_type() } else { self.infer_expr(expr)? }) } else { None }
                } else {
                    None
                };
